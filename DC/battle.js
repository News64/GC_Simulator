...
function save_deck(num_cards, team_num, mode){
	...
	if (document.getElementById('deckname_'+ team_num.toString()).value == "ハイパーオリバー6000" ||
		document.getElementById('deckname_'+ team_num.toString()).value == "ハイパーオリバー7000"){
		...
		*/
		...
		if (battle_data[attacker].confusion ==  true && Math.random() < 0.5){
			...
			for (var i = 0; i < 3; i++){
				...
				if (temp.length > 2 && temp[2].charAt(0) == '+'){
				...
				else if (attack_skill.search('二連射') != -1 && attack_skill.search('（') == -1){
					console.log("!!");
					...
					if (attack_skill.search('Charmer') != -1)
						dmg_rate = 1.2;
				}
				else{
					switch(attack_skill){
		...
		if (choose_skill == -1){
			...
			if (temp.length > 2 && temp[2].charAt(0) == '+'){
				...
				dmg_rate = 1.0; mp_cost = 1200; twice = 2;
				if (attack_skill.search('Charmer') != -1)
					dmg_rate = 1.2;
			}
			else{
