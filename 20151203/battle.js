...
function init_list(num){
	...
	if (num == 1){
		...
		dodge_skill2 = 'dodge_skill21', poison_skill = 'poison_skill1', attr = 'attr1', red_ex = 'red_ex1', blue_ex = 'blue_ex1';
	}
	else{
		...
		dodge_skill2 = 'dodge_skill22', poison_skill = 'poison_skill2', attr = 'attr2', red_ex = 'red_ex2', blue_ex = 'blue_ex2';
	}
...
function damage_dealer(id1, id2, attack_skill, attack_attr, dmg_rate, reduc_rate, fixed){
	...
	if (attack_skill == "Crush Drain" && damage > battle_data[id2].hp_left)
	...
	while (true){ 
		...
		if(temp.length > 1 && temp[1].charAt(0) == '+'){
			...
			switch (attack_skill){
				...
				case "Soul Slash":
					attack_attr = "Physical";
					dmg_rate = 1.5, hp_cost = 1000 - soul_slash_modifier[attacker];
					battle_data[defender].dodgable = false;
					break;
				case "Crush Drain":
